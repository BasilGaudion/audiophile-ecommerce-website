...
const createReport = (jobs) => {
    ...
    const summary = {};
    Object.values(constants_1.Source).forEach((sourceId) => {
        const sourceSummary = jobs[0]?.summary?.[sourceId];
        if (!sourceSummary) {
            return;
        }
        const summaryEntries = Object.entries(sourceSummary).map(([metricId, summaryData]) => {
            ...
            const data = (0, metrics_1.getMetricRunInfo)(metric, current, baseline);
